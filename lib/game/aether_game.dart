...
class _HitFx {
  ...
  double age = 0; static const life = 0.42;
...
class AetherGame extends FlameGame {
  ...
  final Paint _hold = Paint()..color = const Color(0xFF2A2A2A); final Paint _holdOn = Paint()..color = const Color(0xFF404040);
  ...
  final List<_HitFx> _fx = []; static const _fxCap = 10; double _judAge = 1; final Set<int> _pressedLanes = {};
  ...
  void _onJudgment(note, Judgment j, double d) {
    if (!kIsWeb && (j == Judgment.perfect || !settings.reducedVfx)) haptics.forJudgment(j.label);
    if (j == Judgment.miss) { sfx?.playMiss(); } else { sfx?.playHit(j.label); }
    _judAge = 0;
    final lanes = laneCount;
    final lane = settings.handedness == Handedness.left ? (lanes - 1 - note.note.lane % lanes) : note.note.lane % lanes;
    final milestone = _isMilestone(controller.combo.value);
  ...
  Future<void> restart() async { _finishNotified = false; _fx.clear(); _judAge = 1; await clock.stop(); await clock.seek(0); await onLoad(); await clock.play(); _started = true; }
  ...
  @override void update(double dt) {
    ...
    _judAge = math.min(_judAge + dt, 1.0); for (final f in _fx) { f.age += dt; } _fx.removeWhere((f) => f.dead);
  ...
  @override void render(Canvas canvas) {
    ...
    for (final n in controller.visibleNotes()) {
      ...
      if (note.type == NoteType.hold) {
        ...
        final paint = n.holdActive ? _holdOn : _hold; paint.strokeWidth = tw * 0.82; paint.strokeCap = StrokeCap.round;
        canvas.drawLine(Offset(cx, y), Offset(cx, hitLineY - ep * travel), paint);
      }
      final scale = n.holdActive ? 1.04 : 1.0;
      final rr = RRect.fromRectAndRadius(Rect.fromCenter(center: Offset(cx, y), width: tw * scale, height: th * scale), const Radius.circular(7));
      ...
      if (note.type == NoteType.swipe) {
        final tip = TextPainter(textDirection: TextDirection.ltr, text: const TextSpan(text: '▲', style: TextStyle(color: Color(0x88FFFFFF), fontSize: 14)))..layout();
        tip.paint(canvas, Offset(cx - tip.width / 2, y - tip.height / 2));
      }
    }
    _paintFx(canvas, th); _paintHud(canvas); _paintJud(canvas); _paintAp(canvas);
  }

  void _paintFx(Canvas canvas, double th) {
    for (final f in _fx) {
      final fade = 1.0 - f.t; if (fade <= 0) continue;
      if (!settings.reducedVfx || f.judgment == Judgment.perfect) {
        final punch = 1.0 + (1.0 - f.t) * 0.18; final tw = tileW(f.lane) * punch;
  ...
  void _paintHud(Canvas canvas) {
    ...
    if (_hud == null || s != _ls || c != _lc || (a - _la).abs() > 0.0005 || (m - _lm).abs() > 0.01) {
      _ls = s; _lc = c; _la = a; _lm = m;
  ...
  void _paintJud(Canvas canvas) {
    ...
    final punch = 1.0 + (1.0 - _easeOut(_judAge)) * 0.22; final fade = (1.0 - _judAge).clamp(0.0, 1.0);
  ...
  void _paintAp(Canvas canvas) {
    ...
    _ap!.paint(canvas, Offset(size.x - _ap!.width - 16, 22));
  }
}
